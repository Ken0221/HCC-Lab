...
def test_init(PS):
  ...
  PS.sound = pygame.mixer.Sound("exp_img/done.mp3")
  # Drone control
...
# 左右(看/動) 控制左右
def determine_lr(EEG, csp1, csp2):
  EEG_data = EEG[np.newaxis, :, 500:1000] # 取前 0.5~1秒 ，並新增一個 trials 軸
  y_pred1 = csp1.predict(EEG_data)[0]
  if y_pred1==0:
    print('    L/R : rest')
    return 0
  y_pred2 = csp2.predict(EEG_data)[0]
  LR = 'Left' if y_pred2==1 else 'Right'
  LR_ct = C_text(f'{LR}','lc')
  print(f'    L/R : {LR}')
  return y_pred2
...
# 控制指令
def control_command(PS, comm_ud, comm_land, comm_backward, comm_lr, comm_forward):
  ...
  if comm_backward:
    print(green_text.format('backward'))
    return command_dict['backward']
  if comm_forward:
...
def test_phase(PS, trial_num):
  ...
  test_init(PS)
  csp1 = joblib.load(f'EEG_data/{PS.user_name}_model1.joblib') # 判斷是不是rest
  csp2 = joblib.load(f'EEG_data/{PS.user_name}_model2.joblib') # 判斷是左是右
  for i in range(trial_num):
    ...
    # command function
    print("  state : ")
    comm_ud = determine_updown(EEG, PS)           # 眨眼 控制上下
    comm_land = determine_land(EEG, PS)           # 閉眼 控制降落
    comm_backward = determine_backward(EEG, PS)   # 咬牙 控制後退
    comm_lr = determine_lr(EEG, csp1, csp2)     # 左右(看/動) 控制左右0 #
    comm_forward = determine_forward(EEG, PS)     # 專注 控制前進
    # control
    command = control_command(PS, comm_ud, comm_land, comm_backward, comm_lr, comm_forward)
