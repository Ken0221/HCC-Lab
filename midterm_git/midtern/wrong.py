...
if __name__ == '__main__':
    ...
    #         currtime=time.time()
    #         if currtime-lastime >= 6:
    #             fname="asd.jpg"
    #             cv2.imwrite(fname,frame)
    #             recvThread = threading.Thread(target=recv3)
    #             recvThread.start()
    #             lastime=currtime
    ...
    # def recv3():
    #     print("====================")
    #     print("save fig and yolo")
    #     print("====================")
    #     main(opt)
    def run_opt():
        main(opt)
    recvThread = threading.Thread(target=run_opt)
    recvThread.start()

    message1=["command", "streamon", "speed?","takeoff"]
    #message1=["command", "streamon", "speed?"]
    ...
    def brain_control():
      while True:
          ...
          if mode == '1': # 包含訓練模型
            ...
            test_phase(PS, trial_num, IsDetectMode)
            print('\n' + ' done! '.center(60, '='))
    ...
    def detect_control():
      while True:
          if  ineed == "backpack":
              IsDetectMode = True
              print("====================")
              print("get", ineed,"and go") 
              print("====================")
              if (ineedcoords[0] + ineedcoords[2]) / 2 < 0.5:
                  msg = message2
              else:
                  msg = message3
              for i in range(0,len(message2)):
                  msg2=msg[i]
                  sock.sendto(msg2.encode("utf-8"), tello_address1)
                  time.sleep(delay2[i])
          ...
          if  ineed == "suitcase":
              ...
              for i in range(0,len(message3)):
                  msg3=msg[i]
                  sock.sendto(msg3.encode("utf-8"), tello_address1)
                  time.sleep(delay2[i])



          if  ineed != "umbrella" and ineed != "suitcase":
              IsDetectMode = False
              print("no action")
              time.sleep(4)
    recvThread = threading.Thread(target=detect_control)
    recvThread.start()